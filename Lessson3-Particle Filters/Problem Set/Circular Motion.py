...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if beta*beta > 0.000001:
            R = motion[1]/beta
            self.x -= sin(self.orientation)*R-sin(self.orientation+beta)*R
            self.y += cos(self.orientation)*R-cos(self.orientation+beta)*R
            self.orientation += beta
        else:
            self.x += motion[1]*cos(motion[0])
            self.y += motion[1]*sin(motion[0])
            self.orientation += beta
