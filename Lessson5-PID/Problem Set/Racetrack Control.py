...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    #     if self.x< radius  and self.orientation <= 0.5*pi:
    #         cte = self.y-(sqrt(radius**2-(self.x-radius)**2)+radius)
    #     elif radius <= self.x <= 3*radius  and  (self.orientation<0.5*pi or self.orientation>1.5*pi ):
    #         cte = self.y - radius*2
    #     elif self.x>3*radius and self.orientation >= 1.5*pi:
    #         cte = self.y-(sqrt(radius**2-(self.x-3*radius)**2)+radius)
    #     elif self.x>3*radius and self.orientation <= 1.5*pi:
    #         cte = self.y-(sqrt(radius**2-(self.x-3*radius)**2)-radius)
    #     elif radius <= self.x <= 3*radius  and  0.5*pi < self.orientation < 1.5*pi :
    #         cte = -self.y
    #     elif self.x< radius  and self.orientation > 0.5*pi:
    #         cte = -self.y+(sqrt(radius**2-(self.x-radius)**2)-radius)
    #     return cte
