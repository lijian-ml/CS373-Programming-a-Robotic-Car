    ...
    for i in range(N-1):
    	if len(data[i][0]) >0:
	    	for e in data[i][0]:
	    		...
	    		Omega.value[2*i][2*(N+e[0])] += -1
	    		Xi.value[2*i][0] += -e[1]

	    		Omega.value[2*(N+e[0])][2*i] += -1
	    		Omega.value[2*(N+e[0])][2*(N+e[0])] += 1
	    		Xi.value[2*(N+e[0])][0] += e[1]

	    		Omega.value[2*i+1][2*i+1] += 1
	    		Omega.value[2*i+1][2*(N+e[0])+1] += -1
	    		Xi.value[2*i+1][0] += -e[2]

	    		Omega.value[2*(N+e[0])+1][2*i+1] += -1
	    		Omega.value[2*(N+e[0])+1][2*(N+e[0])+1] += 1
	    		Xi.value[2*(N+e[0])+1][0] += e[2]
    	...
    	Omega.value[2*(i+1)][2*(i+1)] += 1
    	Xi.value[2*(i+1)][0] += data[i][1][0]

    	Omega.value[2*i+1][2*i+1] += 1
    	Omega.value[2*i+1][2*(i+1)+1] += -1
    	Xi.value[2*i+1][0] += -data[i][1][1]

    	Omega.value[2*(i+1)+1][2*i+1] += -1
    	Omega.value[2*(i+1)+1][2*(i+1)+1] += 1
    	Xi.value[2*(i+1)+1][0] += data[i][1][1]
    
    mu = Omega.inverse()*Xi
    return mu # Make sure you return mu for grading!
        
############### ENTER YOUR CODE ABOVE HERE ###################
#data = [Z, [dx, dy] for i in range(move)]
#Z = [[i, dx, dy] for i in range(self.num_landmarks)]
# ------------------------------------------------------------------------
# ------------------------------------------------------------------------
# ------------------------------------------------------------------------
#
