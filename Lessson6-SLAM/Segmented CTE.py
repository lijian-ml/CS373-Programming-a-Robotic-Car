...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        estimate_to_node = [estimate[0]-spath[index][0], estimate[1]-spath[index][1]]
        node_to_node = [spath[index+1][0]-spath[index][0], spath[index+1][1]-spath[index][1]]
        ratio = (estimate_to_node[0]*node_to_node[0] +estimate_to_node[1]*node_to_node[1]) / (node_to_node[0]**2+node_to_node[1]**2)
        if  ratio > 1:
            index += 1
            ...
            node_to_node = [spath[index+1][0]-spath[index][0], spath[index+1][1]-spath[index][1]]
            ratio = (estimate_to_node[0]*node_to_node[0] +estimate_to_node[1]*node_to_node[1]) / (node_to_node[0]**2+node_to_node[1]**2)
        cte_flag = 1 if estimate_to_node[1]*node_to_node[0] - estimate_to_node[0]*node_to_node[1] > 0 else -1
        cte = cte_flag*sqrt(estimate_to_node[0]**2+estimate_to_node[1]**2-ratio**2*(node_to_node[0]**2+node_to_node[1]**2))
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise, 
    ...
    for e in path.spath:
        if 1 in [1 if isnan(q) else 0 for q in e]:
            return [1, 100000, 100000]
        if abs(sum(e)) > 1.0e+10 :
            return [1, 100000, 100000]
