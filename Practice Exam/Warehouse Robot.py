...
def plan(warehouse, dropzone, todo):
    state = [[1,0],[0,1],[-1,0],[0,-1]]
    state_decline = [[1,1],[-1,1],[-1,-1],[1,-1]]
    cost = 0

    for ii in range(len(todo)):
      warehouse_flag = [[0 for q in e] for e in warehouse]
      next_state = [dropzone]
      while 1:
        next_state1 = []
        for i in range(len(next_state)):
          for j in range(4):
            first_d = next_state[i][0]+state[j][0]
            second_d = next_state[i][1]+state[j][1]
            first_d_decline = next_state[i][0]+state_decline[j][0]
            second_d_decline = next_state[i][1]+state_decline[j][1]
            if 0<= first_d_decline <len(warehouse) and 0<= second_d_decline <len(warehouse[0]) and warehouse_flag[first_d_decline][second_d_decline]==0 and not(first_d_decline==dropzone[0] and second_d_decline==dropzone[1]):
              warehouse_flag[first_d_decline][second_d_decline] = warehouse_flag[next_state[i][0]][next_state[i][1]]+1.5
              if warehouse[first_d_decline][second_d_decline] ==0:
                next_state1 = [[first_d_decline,second_d_decline]] + next_state1
            if 0<= first_d <len(warehouse) and 0<= second_d <len(warehouse[0]) and warehouse_flag[first_d][second_d]==0 and not(first_d==dropzone[0] and second_d==dropzone[1]):
              warehouse_flag[first_d][second_d] = warehouse_flag[next_state[i][0]][next_state[i][1]]+1.0
              if warehouse[first_d][second_d] ==0:
                next_state1 += [[first_d,second_d]]
          print  next_state,next_state1
        next_state = [next_state1[len(next_state1)-1-i] for i in range(len(next_state1))]
        if next_state == []:
          break      
      for i in range(len(warehouse)):
        for j in range(len(warehouse[i])):
          if warehouse[i][j] == todo[ii]:
            cost += 2*warehouse_flag[i][j]
            warehouse[i][j] = 0
    return int(cost)
